    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                 ...
                 *
                 * storage_path() covers everything storage holds in this app: the
                 * central 'app' disk, and every stancl/tenancy per-tenant directory
                 * (storage/tenant<id>/...), since tenancy's FilesystemTenancyBootstrapper
                 * only suffixes storage_path() at request time — a CLI run of
                 * backup:run sees the true, unsuffixed base storage directory, so one
                 * entry here sweeps up every tenant's media without listing them.
                 *
                 * database_path('tenants') covers the tenant SQLite files themselves.
                 * They can't go through the 'databases' source below because that only
                 * dumps statically configured connections (config/database.php), not
                 * the per-tenant connections stancl/tenancy creates dynamically — so
                 * they're backed up as raw file copies instead.
                 */
                'include' => [
                    storage_path(),
         ...
         *
         * Kept inside storage/framework (already excluded from the source above)
         * rather than storage/app, so the working files this process creates
         * mid-run never get swept into the very backup it's building.
         */
